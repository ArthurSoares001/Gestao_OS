...
procedure TTecnico.ocopy(AObject: TOrObject);
begin
  inherited ocopy(AObject);
  with AObject as TTecnico do
  begin
    self.id := getId;
    self.nome := getNome;
    self.email := getEmail;
    self.telefone := getTelefone;
    self.especialidade := getEspecialidade;
    self.custoHora := getCustoHora;
    self.ativo := getAtivo;
